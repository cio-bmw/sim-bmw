...
function loadData(){ 
  ...
  $.ajax({ 
    url: "txnpos_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
 ...
 function sendtxnpos(n1,n2) 
 {
     document.getElementById('txnpos_idtxnpos').value=n1;
     document.getElementById('posname').value=n2;
     $("#divLOV").hide(); 
 } 
</script> 
...
$fieldcari = $_POST['fieldcari']; 
  ...
  $sql = "select * from txnpos where idtxnpos like '%".$fieldcari."%'";  
...
?> 
	...
	<body> 
	<form method="post" name="txnpos_lov" action="" id="txnpos_lov">  
	<table>  
	<tr><td colspan=5 > 
...
<td><input class="button" type=button value="Pilih" onClick="sendtxnpos('<? echo $row['idtxnpos'];?>','<? echo $row['posname'];?>  ' )"></td>
  		<?} //end while ?> 
