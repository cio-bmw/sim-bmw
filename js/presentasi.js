$(document).ready(function(){ 
	 
	//menangkap error dan men-set parameter global (timeout, dll) 
	$.ajaxSetup({ 
	  timeout: 10000, 
	  cache: false, 
		error:function(x,e){ 
			if(x.status==0){ 
			alert('Anda sedang offline! Silahkan cek koneksi anda!'); 
			}else if(x.status==404){ 
			alert('Permintaan URL tidak ditemukan!'); 
			}else if(x.status==500){ 
			alert('Internal Server Error!'); 
			}else if(e=='parsererror'){ 
			alert('Error.Parsing JSON Request failed!'); 
			}else if(e=='timeout'){ 
			alert('Request Time out!'); 
			}else { 
			alert('Error tidak diketahui: '+x.responseText); 
			} 
		} 
	}); 
	 
	// menampilkan image untuk menandakan proses Ajax sedang berlangsung atau telah selesai  
	$('#divLoading').ajaxStart(function(){ 
		$(this).fadeIn(); 
		$(this).html("<table><tr><td><img src='images/ajax-loader.gif' /></td></tr></table>"); 
	}).ajaxStop(function(){ 
		$(this).fadeOut(); 
	}); 
	 

   
	$("#btnexit").click(function(){ 
		window.location='index.php'; 
	}); 
	 
 	//menampilkan list data pelanggan 
 	loadData1(); 
 	loadData2(); 
 	loadData3(); 
 	loadData4(); 
 	loadData5(); 
 	//loadData6(); 
 	loadData7(); 
 	loadData8(); 
 	 	loadData9();
 	
 	
 	
	 
  // fungsi untuk me-load tampilan list data pelanggan, data yang ditampilkan disesuaikan 
  // juga dengan input data pada bagian search 
  function loadData1(){ 
 	  var dataString; 
	  var vpoli = $("input#poli1").val(); 
	  var vdisplay ='#'+$("input#display1").val(); 
	
    dataString = 'poli='+vpoli;  
    
   $.ajax({ 
   url: "antrian_display1.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$(vdisplay).html(data); 
 		} 
   }); 
    } 
 
 
 function loadData2(){ 
	  var dataString; 
	  var vpoli = $("input#poli2").val(); 
	  var vdisplay ='#'+$("input#display2").val(); 
	
    dataString = 'poli='+vpoli;  
    
   $.ajax({ 
   url: "antrian_display2.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$(vdisplay).html(data); 
 		} 
   }); 
 } 
   
function loadData3(){ 
	  var dataString; 
	  var vpoli = $("input#poli3").val(); 
	  var vdisplay ='#'+$("input#display3").val(); 
	
    dataString = 'poli='+vpoli;  
    
   $.ajax({ 
   url: "antrian_display3.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$(vdisplay).html(data); 
 		} 
   }); 
 }    
   
function loadData4(){ 
	  var dataString; 
	  var vpoli = $("input#poli4").val(); 
	  var vdisplay ='#'+$("input#display4").val(); 
	
    dataString = 'poli='+vpoli;  
    
   $.ajax({ 
   url: "antrian_display4.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$(vdisplay).html(data); 
 		} 
   }); 
 }       
   
function loadData5(){ 
	  var dataString; 
	  var vpoli = $("input#poli5").val(); 
	  var vdisplay ='#'+$("input#display5").val(); 
	
    dataString = 'poli='+vpoli;  
    
   $.ajax({ 
   url: "antrian_display5.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$(vdisplay).html(data); 
 		} 
   }); 
 }       
   
function loadData6(){ 
  var dataString; 
	     
   $.ajax({ 
   url: "antrian_slideatas.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$('#tengahatas').html(data); 
 		} 
   }); 
 }       
 
 function loadData7(){ 
  var dataString; 
	     
   $.ajax({ 
   url: "antrian_video.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$('#tengahtengah').html(data); 
 		} 
   }); 
 }         
   
function loadData8(){ 
  var dataString; 
	     
   $.ajax({ 
   url: "antrian_slide.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$('#tengahbawah').html(data); 
 		} 
   }); 
 }         
  
  function loadData9(){ 
  var dataString; 
	     
   $.ajax({ 
   url: "antrian_runningtext.php", 
     type: "GET", 
     data: dataString, 
 		success:function(data) 
		{ 
			$('#footer').html(data); 
 		} 
   }); 
 }         
   
 
   
 // melakukan pemrosesan data untuk bagian search (pencarian data) 
 $("form#formSearch").submit(function(){  
   var cari = $("input#fieldcari").val(); 
   var combo = $("select#pilihcari").val(); 
   if (cari.replace(/\s/g,"") != ""){ // mengecek field text kosong atau tidak) 
       loadData1(); 
   } 
   else if ((cari.replace(/\s/g,"") == "") && (combo != "semua") ){ 
     alert("Maaf, field harus diisi!"); 
     $("input#fieldcari").focus(); 
   } 
   else{ 
     loadData1(); 
     
   } 
   return false; 
  }); 
   
}); 
