...
function loadData(){ 
  ...
  $.ajax({ 
    url: "receivedtl_dspproduct.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$("#btncarilov").click(function(){ 
 	...
 	page="receivedtl_dspproduct.php?id="+$("input#idreceivehdr").val()+"&fieldcari="+$("input#fieldcari").val();
		$("#divLOV").load(page); 
		$("#divLOV").show(); 
		$("#btnhide").show(); 
		return false; 
...
<td><a href="receivedtl_form.php?action=addproduct&product=<? echo $row['idproduct'];?>&price=<? echo $row['costprice'];?>&idhdr=<? echo $id; ?>" class="pilih"><input type="button" class="button" value="Pilih"></a></td></tr> 
