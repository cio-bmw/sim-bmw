...
function loadData(){ 
  ...
  $.ajax({ 
    url: "unitstatus_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
 ...
 function sendunitstatus(n1,n2) 
 {
     window.opener.document.getElementById('unitstatus_idunitstatus').value=n1;
     window.opener.document.getElementById('unitstatusname').value=n2;
     window.close();
 } 
</script> 
...
$fieldcari = $_POST['fieldcari']; 
  ...
  $sql = "select * from unitstatus where idunitstatus like '%".$fieldcari."%'";  
...
?> 
	...
	<script type="text/javascript" src="js/unitstatus.js"></script> 
	</head> 
	<body> 
...
<td><input class="button" type=button value="Pilih" onClick="sendunitstatus('<? echo $row['idunitstatus'];?>','<? echo $row['unitstatusname'];?>  ' )"></td>
  		<?} //end while ?> 
