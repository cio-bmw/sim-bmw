...
function pagination1(page){ 
  ...
  $.ajax({ 
    url:"unithistorypayment_lov.php", 
    data: dataString, 
		type:"GET", 
...
function loadData(){ 
  ...
  $.ajax({ 
    url: "unithistorypayment_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
  ...
  $("#btncari").click(function(){  
  		page1="unithistorypayment_lov.php?cari="+$("input#carilov").val();  
  		$("#divLOV").load(page1);  
  		$("#divLOV").show();  
     	return false;  
  	});   
 ...
 function sendunithistorypayment(n1,n2) 
 {
     document.getElementById('unithistorypayment_idunithistorypayment').value=n1;
     document.getElementById('unithistorypayment').value=n2;
 } 
</script> 
...
$cari = $_GET['cari']; 
  ...
  $sql = "select * from unithistorypayment where idunithistorypayment like '%".$cari."%'";  
...
?> 
	...
	<p>
	<form method="post" name="unithistorypayment_lov" action="" id="unithistorypayment_lov">  
...
<td><input class="button" type=button value="Pilih" onClick="sendunithistorypayment('<? echo $row['idunithistorypayment'];?>','<? echo $row['unithistorypayment'];?>  ' )"></td>
  		<?} //end while ?> 
