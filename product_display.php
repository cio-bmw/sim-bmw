...
if (isset($_GET['idproduct']) and !empty($_GET['idproduct'])){ 
 $idproduct = $_GET['idproduct']; 
  $sql = "select * from product where idproduct like '%$idproduct%' order by idproduct"; 
} 
else if (isset($_GET['productname']) and !empty($_GET['productname'])){ 
 $productname = $_GET['productname']; 
  $sql = "select * from product where productname like '%$productname%' order by productname"; 
} 
else if (isset($_GET['uom_iduom']) and !empty($_GET['uom_iduom'])){ 
 $uom_iduom = $_GET['uom_iduom']; 
  $sql = "select * from product where uom_iduom like '%$uom_iduom%' order by uom_iduom"; 
} 
else if (isset($_GET['category_idcat']) and !empty($_GET['category_idcat'])){ 
 $category_idcat = $_GET['category_idcat']; 
  $sql = "select * from product where category_idcat like '%$category_idcat%' order by category_idcat"; 
} 
else if (isset($_GET['supplier_idsupp']) and !empty($_GET['supplier_idsupp'])){ 
 $supplier_idsupp = $_GET['supplier_idsupp']; 
  $sql = "select * from product where supplier_idsupp like '%$supplier_idsupp%' order by supplier_idsupp"; 
} 
else if (isset($_GET['location_idlocation']) and !empty($_GET['location_idlocation'])){ 
 $location_idlocation = $_GET['location_idlocation']; 
  $sql = "select * from product where location_idlocation like '%$location_idlocation%' order by location_idlocation"; 
} 
else if (isset($_GET['salesprice']) and !empty($_GET['salesprice'])){ 
 $salesprice = $_GET['salesprice']; 
  $sql = "select * from product where salesprice like '%$salesprice%' order by salesprice"; 
} 
else if (isset($_GET['costprice']) and !empty($_GET['costprice'])){ 
 $costprice = $_GET['costprice']; 
  $sql = "select * from product where costprice like '%$costprice%' order by costprice"; 
} 
else if (isset($_GET['stock']) and !empty($_GET['stock'])){ 
 $stock = $_GET['stock']; 
  $sql = "select * from product where stock like '%$stock%' order by stock"; 
} 
else if (isset($_GET['stockwh']) and !empty($_GET['stockwh'])){ 
 $stockwh = $_GET['stockwh']; 
  $sql = "select * from product where stockwh like '%$stockwh%' order by stockwh"; 
} 
else if (isset($_GET['limitstock']) and !empty($_GET['limitstock'])){ 
 $limitstock = $_GET['limitstock']; 
  $sql = "select * from product where limitstock like '%$limitstock%' order by limitstock"; 
} 
else if (isset($_GET['limitstockwh']) and !empty($_GET['limitstockwh'])){ 
 $limitstockwh = $_GET['limitstockwh']; 
  $sql = "select * from product where limitstockwh like '%$limitstockwh%' order by limitstockwh"; 
} 
else if (isset($_GET['status']) and !empty($_GET['status'])){ 
 $status = $_GET['status']; 
  $sql = "select * from product where status like '%$status%' order by status"; 
} 
else if (isset($_GET['active']) and !empty($_GET['active'])){ 
 $active = $_GET['active']; 
  $sql = "select * from product where active like '%$active%' order by active"; 
} 
else if (isset($_GET['boxqty']) and !empty($_GET['boxqty'])){ 
 $boxqty = $_GET['boxqty']; 
  $sql = "select * from product where boxqty like '%$boxqty%' order by boxqty"; 
} 
else{ 
...
<td class="right"><? echo nf($row['limitstock']);?></td>
        ...
        <td width="180px">
       | <a href="product_history.php?id=<? echo $row['idproduct'];?>" class="detail"> <input type="button" class="button" value="History"></a>  
       | <a href="product_form.php?action=update&id=<? echo $row['idproduct'];?>" class="edit"> <input type="button" class="button" value="Edit"></a>   
       | <a href="product_process.php?action=delete&id=<? echo $row['idproduct'];?>" class="delete"><input type="button" class="button" value="Delete"></a></td></tr> 
  		<?} //end while ?> 
