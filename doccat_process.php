<? 
  ...
  $action=$_POST['action']; 
 $iddoccat=$_POST['iddoccat']; 
 $doccatname=$_POST['doccatname']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data doccat 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into doccat (iddoccat,doccatname)  values  ('$iddoccat','$doccatname')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data doccat 
	{ 
$sql = " update doccat set iddoccat='$iddoccat',doccatname='$doccatname' where iddoccat = $iddoccat";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data doccat 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from doccat where iddoccat ='$id'"); 
