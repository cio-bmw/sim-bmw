<? 
  ...
  $action=$_POST['action']; 
 $idaset=$_POST['idaset']; 
 $asetname=$_POST['asetname']; 
 $asetvalue=$_POST['asetvalue']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data aset 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into aset (idaset,asetname,asetvalue)  values  ('$idaset','$asetname','$asetvalue')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data aset 
	{ 
$sql = " update aset set idaset='$idaset',asetname='$asetname',asetvalue='$asetvalue' where idaset = $idaset";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data aset 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from aset where idaset ='$id'"); 
