...
function loadData(){ 
 var dataString; 
  ...
  $.ajax({ 
    url: "receivedtl_display.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString,
		success:function(data) 
		{ 
			...
			$.ajax({ 
				...
				{ 
					if(response.status == 1){ 
						loadData(); 
						$("#divFormEntry").load("receivedtl_form.php"); 
						$("#divFormEntry").hide();
...
$check=mysql_fetch_array(mysql_query("select rcv_status from receivehdr where idreceivehdr = '$id'"));
$status = $check[0];
...
<td class="right"><? echo nf($total);?></td>
        ...
        <td width="120px">
       <? if ($status == 'open') { ?>
        <a href="receivedtl_form.php?action=update&id=<? echo $row['idreceivedtl'];?>" class="edit"> <input type="button" class="button" value="Edit"></a>   
       | <a href="receivedtl_process.php?action=delete&id=<? echo $row['idreceivedtl'];?>" class="delete"><input type="button" class="button" value="Delete"></a>
       <? } else { ?>
        <a href="receivedtl_form.php?action=update&id=<? echo $row['idreceivedtl'];?>" class="noaction"> <input type="button" class="button" value="Edit"></a>   
       | <a href="receivedtl_process.php?action=delete&id=<? echo $row['idreceivedtl'];?>" class="noaction"><input type="button" class="button" value="Delete"></a>
