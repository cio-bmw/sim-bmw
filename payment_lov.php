...
function loadData(){ 
  ...
  $.ajax({ 
    url: "payment_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
 ...
 function sendpayment(n1,n2) 
 {
     window.opener.document.getElementById('payment_idpayment').value=n1;
     window.opener.document.getElementById('payment').value=n2;
     window.close();
 } 
</script> 
...
$fieldcari = $_POST['fieldcari']; 
  ...
  $sql = "select * from payment where idpayment like '%".$fieldcari."%'";  
...
<td><input class="button" type=button value="Pilih" onClick="sendpayment('<? echo $row['idpayment'];?>','<? echo $row['payment'];?>  ' )"></td>
  		<?} //end while ?> 
