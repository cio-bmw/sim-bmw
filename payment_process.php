<? 
 ...
 $paymentdesc=$_POST['paymentdesc']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data payment 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into payment (idpayment,paymentdesc)  values  ('$idpayment','$paymentdesc')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data payment 
	{ 
$sql = " update payment set idpayment='$idpayment',paymentdesc='$paymentdesc' where idpayment = $idpayment";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data payment 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from payment where idpayment ='$id'"); 
