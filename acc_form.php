...
$(function(){ 
 ...
 $("input").not($(":submit")).keypress(function (evt) {  
  ...
  $("form#acc_form").submit(function(){ 
    if (confirm("Apakah benar akan menyimpan data acc ini?")){ 
    		$.ajax({ 
        	url: "acc_process.php", 
        	...
        	success:function(response){ 
              ...
              loadData(); //reload list data 
          		$("#divPageEntry").load("acc_form.php");   
              $("#divPageEntry").hide(); 
            } 
          else if(response.status == 1) { 
          	  alert("Data Baru berhasil disimpan!"); 
              loadData(); //reload list data 
         //   window.location="acc_detail.php?id="+$("input#idslshdr").val(); 
                    		} 
          	else 
          	{ 
...
<p class="judul">Form Memasukkan / Edit Data Account</p>  
<table> 
<tr><th colspan="2"><b><?php echo $judul; ?></b></th></tr> 
<tr> 
<td class="right">Group Account</td> 
<td>
<select id="groupacc_idgroupacc" name="groupacc_idgroupacc">
<? creategroupaccoption($groupacc_idgroupacc); ?>
</select> </td> 
</tr> 
...
<?php } else {?> 
<tr><td class="right">idacc</td><td><input type="text" id="idacc" name="idacc" size="10" value="<? echo $idacc;?>" /></td> 
</tr> 
<?php }?> 

<tr> 
<td class="right">accname</td> 
<td><input type="text" id="accname" name="accname" size="30" maxlength="25" value="<? echo $accname;?>" /></td> 
</tr> 
