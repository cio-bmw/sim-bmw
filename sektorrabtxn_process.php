<? 
 ...
 $rabmst_idrabmst=$_POST['rabmst_idrabmst']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data sektorrabtxn 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into sektorrabtxn (idtxn,txndate,txnvalue,txndesc,sektor_idsektor,rabmst_idrabmst)  values  ('$idtxn','$txndate','$txnvalue','$txndesc','$sektor_idsektor','$rabmst_idrabmst')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data sektorrabtxn 
	{ 
$sql = " update sektorrabtxn set idtxn='$idtxn',txndate='$txndate',txnvalue='$txnvalue',txndesc='$txndesc',sektor_idsektor='$sektor_idsektor',rabmst_idrabmst='$rabmst_idrabmst' where idtxn = $idtxn";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data sektorrabtxn 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from sektorrabtxn where idtxn ='$id'"); 
