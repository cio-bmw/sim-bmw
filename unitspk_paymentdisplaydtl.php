...
?> 
  ...
  <table id="spkpaymentdtl" class="grid"> 
  <tr> 
 <th>No</th>
<th>No SPK</th>
<th>Kategori SPK</th>
...
<td class="right"><? echo nf($row['payvalue']);?></td>
        ...
        <td width="105px"><a href="spkpaymentdtl_form.php?action=update&id=<? echo $row['idspkpaymentdtl'];?>" class="edit"> <input type="button" class="button" value="Edit"></a>   
       | <a href="spkpaymentdtl_process.php?action=delete&id=<? echo $row['idspkpaymentdtl'];?>" class="delete"><input type="button" class="button" value="Delete"></a></td></tr> 
  		<?  $no++;
          $total= $total+$row['payvalue'];    		
  		
  		} //end while ?> 
		 <tr><td colspan="5">Total</td><td class="right"><?php echo nf($total);?></td><td></td></tr> 
