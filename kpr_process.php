<? 
 ...
 $kprname=$_POST['kprname']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data kpr 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into kpr (idkpr,kprname)  values  ('$idkpr','$kprname')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data kpr 
	{ 
$sql = " update kpr set idkpr='$idkpr',kprname='$kprname' where idkpr = $idkpr";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data kpr 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from kpr where idkpr ='$id'"); 
