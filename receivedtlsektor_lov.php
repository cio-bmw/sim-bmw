...
function loadData(){ 
  ...
  $.ajax({ 
    url: "receivedtlsektor_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
 ...
 function sendreceivedtlsektor(n1,n2) 
 {
     window.opener.document.getElementById('receivedtlsektor_idreceivedtl').value=n1;
     window.opener.document.getElementById('receivedtlsektor').value=n2;
     window.close();
 } 
</script> 
...
$fieldcari = $_POST['fieldcari']; 
  ...
  $sql = "select * from receivedtlsektor where idreceivedtlsektor like '%".$fieldcari."%'";  
...
<th>batch_no</th>
<th>exp_date</th>
<th>receivehdrsektor_idreceivehdr</th>
...
<td><? echo $row['receivehdrsektor_idreceivehdr'];?></td>
<td><? echo $row['product_idproduct'];?></td>
 
<td><input class="button" type=button value="Pilih" onClick="sendreceivedtlsektor('<? echo $row['idreceivedtl'];?>','<? echo $row['receivedtlsektor'];?>  ' )"></td>
  		<?} //end while ?> 
