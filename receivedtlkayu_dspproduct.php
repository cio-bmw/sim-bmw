...
function pagination1(page){ 
  ...
  $.ajax({ 
    url:"receivedtlkayu_dspproduct.php", 
    data: dataString, 
		type:"GET", 
...
function loadData(){ 
  ...
  $.ajax({ 
    url: "receivedtlkayu_dspproduct.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$("#btncarilov").click(function(){ 
 	...
 	page="receivedtlkayu_dspproduct.php?id="+$("input#idreceivehdr").val()+"&fieldcari="+$("input#fieldcari").val();
		$("#divLOV").load(page); 
		$("#divLOV").show(); 
		$("#btnhide").show(); 
		return false; 
...
$sql = "select * from productkayu where productname like '%".$fieldcari."%'";  
...
<td><input class="button" type=button value="Pilih" onClick="sendpilih('<? echo $row['idproduct'];?>','<? echo $row['productname'];?>','<? echo $row['costprice'];?>' )"></td>
