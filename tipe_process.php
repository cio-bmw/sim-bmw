<? 
 ...
 $lt=$_POST['lt']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data tipe 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into tipe (idtipe,tipename,ukuran,lb,lt)  values  ('$idtipe','$tipename','$ukuran','$lb','$lt')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data tipe 
	{ 
$sql = " update tipe set idtipe='$idtipe',tipename='$tipename',ukuran='$ukuran',lb='$lb',lt='$lt' where idtipe = $idtipe";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data tipe 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from tipe where idtipe ='$id'"); 
