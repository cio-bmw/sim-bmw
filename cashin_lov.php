...
function loadData(){ 
  ...
  $.ajax({ 
    url: "cashin_lov.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
 ...
 function sendcashin(n1,n2) 
 {
     window.opener.document.getElementById('cashin_idcashin').value=n1;
     window.opener.document.getElementById('cashin').value=n2;
     window.close();
 } 
</script> 
...
$fieldcari = $_POST['fieldcari']; 
  ...
  $sql = "select * from cashin where idcashin like '%".$fieldcari."%'";  
...
<td><input class="button" type=button value="Pilih" onClick="sendcashin('<? echo $row['idcashin'];?>','<? echo $row['cashin'];?>  ' )"></td>
  		<?} //end while ?> 
