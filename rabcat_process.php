<? 
  ...
  $action=$_POST['action']; 
 $idrabcat=$_POST['idrabcat']; 
 $rabcatname=$_POST['rabcatname']; 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data rabcat 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into rabcat (idrabcat,rabcatname)  values  ('$idrabcat','$rabcatname')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data rabcat 
	{ 
$sql = " update rabcat set idrabcat='$idrabcat',rabcatname='$rabcatname' where idrabcat = $idrabcat";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data rabcat 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from rabcat where idrabcat ='$id'"); 
