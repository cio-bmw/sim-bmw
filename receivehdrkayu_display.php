...
function loadData(){ 
  ...
  $.ajax({ 
    url: "receivehdrkayu_display.php", //file tempat pemrosesan permintaan (request) 
    type: "GET", 
    data: dataString, 
		success:function(data) 
		{ 
...
$(function(){  
  ...
  $('#receivehdr tr:odd:not(#nav):not(#total)').addClass('odd'); 
   ...
   $("a.process").click(function(){ 
		window.location='receivedtlkayu.php?id= $row["idreceivehdr"]'; 
	}); 	 
			...
			$.ajax({ 
				...
				{ 
					if(response.status == 1){ 
						loadData(); 
						$("#divFormContent").load("receivehdrkayu_form.php"); 
						$("#divFormContent").hide(); 
            $("#btnhide").hide(); 
...
if (isset($_GET['idreceivehdr']) and !empty($_GET['idreceivehdr'])){ 
 $idreceivehdr = $_GET['idreceivehdr']; 
  $sql = "select * from receivehdrkayu where idreceivehdr like '%$idreceivehdr%' order by idreceivehdr desc"; 
} 
else if (isset($_GET['supplier_idsupp']) and !empty($_GET['supplier_idsupp'])){ 
 $supplier_idsupp = $_GET['supplier_idsupp']; 
  $sql = "select * from receivehdrkayu where supplier_idsupp like '%$supplier_idsupp%' and rcv_status like '%$rcv_status%'  order by supplier_idsupp"; 
} 
else if (isset($_GET['rcv_date']) and !empty($_GET['rcv_date'])){ 
 $rcv_date = $_GET['rcv_date']; 
  $sql = "select * from receivehdrkayu where rcv_date like '%$rcv_date%' order by rcv_date"; 
} 
else if (isset($_GET['rcv_bon']) and !empty($_GET['rcv_bon'])){ 
 $rcv_bon = $_GET['rcv_bon']; 
  $sql = "select * from receivehdrkayu where rcv_bon like '%$rcv_bon%' order by rcv_bon"; 
} 
else if (isset($_GET['rcv_titip']) and !empty($_GET['rcv_titip'])){ 
 $rcv_titip = $_GET['rcv_titip']; 
  $sql = "select * from receivehdrkayu where rcv_titip like '%$rcv_titip%' order by rcv_titip"; 
} 
else if (isset($_GET['rcv_desc']) and !empty($_GET['rcv_desc'])){ 
 $rcv_desc = $_GET['rcv_desc']; 
  $sql = "select * from receivehdrkayu where rcv_desc like '%$rcv_desc%' order by rcv_desc"; 
} 
else if (isset($_GET['due_date']) and !empty($_GET['due_date'])){ 
 $due_date = $_GET['due_date']; 
  $sql = "select * from receivehdrkayu where due_date like '%$due_date%' order by due_date"; 
} 
else if (isset($_GET['paid_date']) and !empty($_GET['paid_date'])){ 
 $paid_date = $_GET['paid_date']; 
  $sql = "select * from receivehdrkayu where paid_date like '%$paid_date%' order by paid_date"; 
} 
else if (isset($_GET['faktur']) and !empty($_GET['faktur'])){ 
 $faktur = $_GET['faktur']; 
  $sql = "select * from receivehdrkayu where faktur like '%$faktur%' order by faktur"; 
} 
else if (isset($_GET['rcv_bayar']) and !empty($_GET['rcv_bayar'])){ 
 $rcv_bayar = $_GET['rcv_bayar']; 
  $sql = "select * from receivehdrkayu where rcv_bayar like '%$rcv_bayar%' order by rcv_bayar"; 
} 
else if (isset($_GET['rcv_status']) and !empty($_GET['rcv_status'])){ 
 $rcv_status = $_GET['rcv_status']; 
  $sql = "select * from receivehdrkayu where rcv_status like '%$rcv_status%' order by rcv_status"; 
} 
else if (isset($_GET['rcv_diskon']) and !empty($_GET['rcv_diskon'])){ 
 $rcv_diskon = $_GET['rcv_diskon']; 
  $sql = "select * from receivehdrkayu where rcv_diskon like '%$rcv_diskon%' order by rcv_diskon"; 
} 
else if (isset($_GET['rcv_totprice']) and !empty($_GET['rcv_totprice'])){ 
 $rcv_totprice = $_GET['rcv_totprice']; 
  $sql = "select * from receivehdrkayu where rcv_totprice like '%$rcv_totprice%' order by rcv_totprice"; 
} 
else if (isset($_GET['rcv_totdiskon']) and !empty($_GET['rcv_totdiskon'])){ 
 $rcv_totdiskon = $_GET['rcv_totdiskon']; 
  $sql = "select * from receivehdrkayu where rcv_totdiskon like '%$rcv_totdiskon%' order by rcv_totdiskon"; 
} 
else if (isset($_GET['rcv_totppn']) and !empty($_GET['rcv_totppn'])){ 
 $rcv_totppn = $_GET['rcv_totppn']; 
  $sql = "select * from receivehdrkayu where rcv_totppn like '%$rcv_totppn%' order by rcv_totppn"; 
} 
else if (isset($_GET['emp_idemp']) and !empty($_GET['emp_idemp'])){ 
 $emp_idemp = $_GET['emp_idemp']; 
  $sql = "select * from receivehdrkayu where emp_idemp like '%$emp_idemp%' order by emp_idemp"; 
} 
else { 
   $sql = "select * from receivehdrkayu order by idreceivehdr desc"; 
} 
...
$result = $obj->result; 
?> 
	<p class="judul">Daftar Penerimaan Kayu</p> 
  <table id="receivehdr" class="grid"> 
  <tr> 
 <th>No Dok</th>
<th>Kode</th>
<th>Nama Supplier</th>
...
$sql2="SELECT sum(receive_price * qty) vreceive FROM receivedtlkayu  
...
<td class="right"><? echo nf($mrcv);?></td>
        ...
        <td width="180px">
           <a href="receivedtlkayu.php?action=detail&id=<? echo $row['idreceivehdr'];?>" class="process"> <input type="button" class="button" value="Detail"></a>   
        <? if ($row['rcv_status'] =="open") { ?>     
           | <a href="receivehdrkayu_form.php?action=update&id=<? echo $row['idreceivehdr'];?>" class="edit"> <input type="button" class="button" value="Edit"></a>   
       | <a href="receivehdrkayu_process.php?action=delete&id=<? echo $row['idreceivehdr'];?>" class="delete"><input type="button" class="button" value="Delete"></a>
          <? } else { ?>
