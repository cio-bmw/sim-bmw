<? 
 ...
 $paymentdesc=ucwords($_POST['paymentdesc']); 
   ...
   $panjang = mysql_field_len($struktur,0); 
	...
	if($action=="add") //menangani aksi penambahan data unitmstpayment 
	{ 
	//  $idpelanggan = buatID("tbl_pelanggan","P"); 
 mysql_query(" insert into unitmstpayment (idpayment,paymentdesc)  values  ('$idpayment','$paymentdesc')")  or die("Data gagal Di Tambahkan!");  
    // mengembalikan respon dalam format JSON. 
    // status "1" berarti proses berhasil dilakukan. 
    echo '{"status":"1"}';   
		exit; 
	} 
	elseif($action=="update") //menangani aksi perubahan data unitmstpayment 
	{ 
$sql = " update unitmstpayment set idpayment='$idpayment',paymentdesc='$paymentdesc' where idpayment = $idpayment";  
		$test = mysql_query($sql); 
		echo '{"status":"1"}'; 
		exit; 
	} 
	elseif($_GET['action']=="delete") //menangani aksi penghapusan data unitmstpayment 
	{ 
		$id = $_GET['id']; 
		$test = mysql_query("delete from unitmstpayment where idpayment ='$id'"); 
